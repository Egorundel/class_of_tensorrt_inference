...
public:
	static void preprocess(const cv::Mat& img, float data[]);
	std::vector<Bbox> rescale_box(const std::vector<Bbox> &out, int width, int height) const;
	cv::Mat renderBoundingBox(cv::Mat image, const std::vector<Bbox> &bboxes) const;
	...
	void detect(const cv::Mat& frame) {
		...
		cudaMemcpy(buffers[0], input, input_size, cudaMemcpyHostToDevice);
...
void TensorRTInference::preprocess(const cv::Mat& img, float data[]) {
	int w;
	int h;
	int x;
	int y;
...
std::vector<TensorRTInference::Bbox> TensorRTInference::rescale_box(const std::vector<Bbox> &out, int width, int height) const{
...
// visualization
cv::Mat TensorRTInference::renderBoundingBox(cv::Mat image, const std::vector<Bbox> &bboxes) const{
...
int main(int argc, char** argv) {
	...
	// Open the video file
	std::string video_filepath = "../sample_videos/video.mp4";
